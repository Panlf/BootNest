package com.boot.plugin.config;
...
public class MyBatisFullTableSearchInterceptor extends JsqlParserSupport implements Interceptor {
    ...
    protected void processSelect(Select select, int index, String sql, Object obj) {
        ...
        String whereStr = where.toString().toLowerCase().trim();
        if("1=1".equals(whereStr) || "true".equals(whereStr)){
            throw new RuntimeException("非法SQL，where 跟随恒等式");
        }
    }
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        //enable=true 启用检查；无注解或 enable=false 才跳过
